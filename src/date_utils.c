...
/*   Updated: 2019/02/21 14:45:59 by frossiny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
time_t	get_ftime(struct stat stats, t_option option)
{
	...
	if (option.c)
		return (stats.st_ctimespec.tv_sec);
	if (option.cu)
		return (stats.st_birthtimespec.tv_sec);
...
char	*build_time(struct stat pstat, t_option option)
{
	char	buf[80];
	time_t	ftime;
